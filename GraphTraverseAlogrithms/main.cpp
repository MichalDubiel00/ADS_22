...
int main() {
	...
	string graph;
	while (true) {
		cout << "\n" << "Praktikum 5: Graphenalgorithem:" << "\n"
			<< "1) Graph einlesen" << "\n"
			<< "2) Tiefensuche" << "\n"
			<< "3) Breitensuche" << "\n"
			<< "4) MST nach Prim" << "\n"
			<< "5) MST nach Kruskal" << "\n"
			<< "6) Kuerzeste Wege nach Dijkstra" << "\n"
		...
		EdgeWeightedDigraph dgra(graph);
		...
		if (input == "2") {
			...
			if (!isInGrapf(gra, such)) {
				cout << "Unzulessiege eingabe" << endl;
				continue;
			}
			...
			DijkstraSP dijkstra(dgra, start);
